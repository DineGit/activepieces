...
import { FlowPlanLimits, DEFAULT_FREE_PLAN_LIMIT } from '.'
...
export const projectLimitsService = {
    async upsert(
        ...
        if (existingPlan) {
            await projectPlanRepo.update(existingPlan.id, planLimits)
        }
        else {
    ...
    async getPlanByProjectIdOrFail(projectId: string): Promise<ProjectPlan> {
        return projectPlanRepo.findOneByOrFail({ projectId })
    },
